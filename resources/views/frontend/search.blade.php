...
<div class="our-product-area">
	<div class="container">
		<div class="area-title">
			<h2>Tìm thấy {{ $product->total() }} kết quả hợp lệ!</h2>
		</div>
		<div class="fillter-product" style="border-bottom: 1px solid #ccc; margin-bottom: 20px;" >
			<div class="row">
				<ul class="filter">
					...
					<li class="frange">
						<a href="{{url()->current()}}?key={{request()->key }}&p=duoi-1-trieu">Dưới 1 triệu</a>
						<a href="{{url()->current()}}?key={{request()->key }}&p=tu-1-3-trieu">Từ 1 - 3 triệu</a>
						<a href="{{url()->current()}}?key={{request()->key }}&p=tu-3-7-trieu">Từ 3 - 7 triệu</a>
						<a href="{{url()->current()}}?key={{request()->key }}&p=tren-7-trieu">Trên 7 triệu</a>
					</li>
					<li>
						<div class="dropdown">
							...
							<div class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdownMenuButton">
								...
								<a class="dropdown-item"
								 href="{{url()->current()}}?key={{request()->key }}&p={{request()->p}}&sort=san-pham-hot">Nổi bật nhất</a>
								<a class="dropdown-item"
								 href="{{url()->current()}}?key={{request()->key }}&p={{request()->p}}&sort=cao-den-thap">Giá cao đến thấp</a>
								<a class="dropdown-item" 
								href="{{url()->current()}}?key={{request()->key }}&p={{request()->p}}&sort=thap-den-cao">Giá thấp đến cao</a>
								@else
								<a class="dropdown-item" href="{{url()->current()}}?key={{request()->key }}&sort=san-pham-hot">Nổi bật nhất</a>
								<a class="dropdown-item" href="{{url()->current()}}?key={{request()->key }}&sort=cao-den-thap">Giá cao đến thấp</a>
								<a class="dropdown-item" href="{{url()->current()}}?key={{request()->key }}&sort=thap-den-cao">Giá thấp đến cao</a>
								@endif
							</div>
						</div>
					</li>
		...
		<div class="row">
			...
			{{ $product->appends(['key'=> request()->key,'p' => request()->p,'sort'=>request()->sort])->links() }}
		    </div>
		</div>
	</div>
</div>
